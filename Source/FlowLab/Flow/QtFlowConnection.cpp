...
QtFlowConnection::QtFlowConnection(QtFlowPin* start_pin, QtFlowPin* end_pin, QGraphicsItem* parent)
...
QtFlowConnection::~QtFlowConnection()
{
}

void QtFlowConnection::set_pin(QtFlowPin* pin)
{
...
void QtFlowConnection::unlink()
{
    if (_start_pin)
    {
        _start_pin->remove_connection(this);
        _start_pin = nullptr;
    }

    if (_end_pin)
    {
        _end_pin->remove_connection(this);
        _end_pin = nullptr;
    }
}

QtFlowPin* QtFlowConnection::start_pin() const
{
    return _start_pin;
}
QtFlowPin* QtFlowConnection::end_pin() const
{
    return _end_pin;
}
int QtFlowConnection::type() const
{
    return Type;
}

QRectF QtFlowConnection::boundingRect() const
{
    qreal extra = 3;
    return QGraphicsPathItem::boundingRect().normalized().adjusted(-extra, -extra, extra, extra);
}
void QtFlowConnection::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
    Q_UNUSED(option);
    Q_UNUSED(widget);
...
void QtFlowConnection::build_path()
{
