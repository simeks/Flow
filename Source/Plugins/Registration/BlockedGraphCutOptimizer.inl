...
void BlockedGraphCutOptimizer<TImage>::execute(
    ...
    ImageVec3d& def)
{
    console::print("Performing registration.\n");
    ...
    console::print("Volume size: %d, %d, %d\n", dims.x, dims.y, dims.z);
    console::print("Block count: %d, %d, %d\n", block_count.x, block_count.y, block_count.z);
    console::print("Block size: %d, %d, %d\n", block_dims.x, block_dims.y, block_dims.z);
    console::print("Block rest: %d, %d, %d\n", block_rest.x, block_rest.y, block_rest.z);
...
#pragma omp parallel for
                for (int block_idx = 0; block_idx < num_blocks; ++block_idx)
                {
                    ...
                    {
                        need_update = need_update || change_flags.is_block_set(block_p + n, use_shift == 1);
                    }
                    ...
                    for (auto& n : _neighbors)
                    {
                        Vec3d delta = _epsilon * Vec3d(
                            moving_spacing_inv.x * n.x,
                            moving_spacing_inv.y * n.y,
                            moving_spacing_inv.z * n.z
                            );
